...
def multiply_numbers(sequence_of_lists):
    ...
    for main_pos in range(len(sequence_of_lists)):
        sublist = sequence_of_lists[main_pos]
        for sub in range(len(sublist)):
            sublist[sub] = sublist[sub] * (main_pos + 1)
...
def run_test_sum_numbers():
    ...
    # Test 3:
    expected = 108
    answer = sum_numbers(([(4,5,6), (1,2,3), (78,9)]))
    print('Expected and actual are:', expected, answer)

    # Test 4
    expected = 8
    answer = sum_numbers(([(1, 1, 1), (1, 1, 1), (1, 1)]))
    print('Expected and actual are:', expected, answer)
...
def sum_numbers(seq_seq):
    ...
    for outer in range(len(seq_seq)):
        sublist = seq_seq[outer]
        for inner in range(len(sublist)):
            total = total + sublist[inner]
    return total
...
def print_characters(sequence_of_strings):
    ...
    for outer in range(len(sequence_of_strings)):
        sublist = sequence_of_strings[outer]
        for inner in range(len(sublist)):
            print(sublist[inner])
